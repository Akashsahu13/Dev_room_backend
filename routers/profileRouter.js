...
router.put("/follower/:profileId", authenticate, async (request, response) => {
  try {

    let profileIds = request.params.profileId;
    var userId = request.user.id;
// for Follower Part
    let pro = await Profile.findOne({ _id: profileIds  });
    var isFollowing = pro.Follower.includes(userId);
    // console.log(postt)
    // console.log(isFollowing);
    // console.log(pro.Follower.length);
    var option = isFollowing ? "$pull" : "$addToSet";
    // Insert/delete post like
    let profiless = await Profile.findOneAndUpdate(
      { _id: profileIds },
...
// ow for following part 

    let fouser= await Profile.findOne({user: userId });
    console.log(fouser);
    console.log(userId);
    console.log(" Next")
  ...
  var option2 = isfollowed ? "$pull" : "$addToSet";
    ...
    let pre = await Profile.findOneAndUpdate(
      { user: userId },
      { [option2]: { Following: pp} },
      { new: true }
    )
    ...
    response.status(200).json({ profiless: profiless });
  } catch (error) {
    console.error(error);
    response.status(500).json({ errors: [{ msg: error.message }] });
  }
